...
container_id=$(docker run $FUSE_DEVICE -e S4_PRIV_KEY="$S4_PRIV_KEY" --cap-add SYS_ADMIN --restart always --name s4-agent-$VOLUME_NAME -v $(pwd):/s4 -d s4-agent:latest "s4 $ACTION")